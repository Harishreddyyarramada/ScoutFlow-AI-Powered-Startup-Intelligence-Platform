...
export default function Companies() {
  ...
  const [sortKey, setSortKey] = useState<"score" | "name" | "foundedYear">(
    "score",
  );
  ...
  const filtered = useMemo(() => {
    let result = companies.filter((c) => {
      if (
        ...
        return false;
      if (filters.sectors.length && !filters.sectors.includes(c.sector))
        return false;
      if (filters.stages.length && !filters.stages.includes(c.stage))
        return false;
      if (filters.locations.length && !filters.locations.includes(c.location))
        return false;
      if (c.score < filters.scoreRange[0] || c.score > filters.scoreRange[1])
        return false;
      return true;
    });
    result.sort((a, b) => {
  ...
  const activeFilterCount =
    filters.sectors.length + filters.stages.length + filters.locations.length;
  ...
  <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-top fade-in duration-300">
    <div className="flex items-center gap-3 rounded-xl border border-green-500/20 bg-green-500/10 backdrop-blur-xl px-5 py-3 shadow-xl">
      ...
      <div>
        ...
        <p className="text-xs text-green-600/80">
          You can access it anytime in Saved Searches
        </p>
      </div>
    </div>
  </div>
          ...
          <Collapsible key={group.key} defaultOpen>
            ...
            <CollapsibleContent className="space-y-0.5 px-2 pb-2">
              {group.options.map((opt) => (
                <label
                  key={opt}
                  className="flex items-center gap-2 rounded px-1 py-1 text-xs cursor-pointer hover:bg-secondary transition-colors"
                >
                  <Checkbox
      ...
      <div className="flex-1 min-w-0 w-full">
          ...
          >
            ...
            <CollapsibleContent className="mt-2 space-y-2 px-2 pb-2">
              {[
                ...
                <div
                  key={`m-${group.key}`}
                  className="rounded-lg bg-background/50 border border-border/30 p-2.5"
                >
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {group.options.map((opt) => (
                      <label
                        key={opt}
                        className="flex items-center gap-2 rounded px-2 py-1.5 text-xs cursor-pointer hover:bg-secondary transition-colors"
                      >
                        <Checkbox
                          checked={filters[group.key].includes(opt)}
                          onCheckedChange={() => toggleFilter(group.key, opt)}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-secondary/50">
                ...
                <th className="hidden sm:table-cell px-2 sm:px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Sector
                </th>
                <th className="hidden sm:table-cell px-2 sm:px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Stage
                </th>
                <th className="hidden lg:table-cell px-2 sm:px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Location
                </th>
                <th className="px-2 sm:px-3 py-2.5 text-left">
                  <button
                ...
                <th className="px-2 sm:px-3 py-2.5 text-right text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody>
              {paged.map((company) => (
                ...
                >
                  ...
                  <td className="hidden sm:table-cell px-2 sm:px-3 py-3 text-muted-foreground text-xs">
                    {company.stage}
                  </td>
                  <td className="hidden lg:table-cell px-2 sm:px-3 py-3 text-muted-foreground text-xs">
                    {company.location}
                  </td>
                  ...
                  <td
                    className="px-2 sm:px-3 py-3 text-right"
                    onClick={(e) => e.stopPropagation()}
                  >
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-muted-foreground text-center sm:text-left order-2 sm:order-1">
            Showing {paged.length === 0 ? 0 : (page - 1) * perPage + 1}–
