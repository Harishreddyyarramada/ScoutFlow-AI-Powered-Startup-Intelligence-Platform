        ...
        <main className="flex-1 min-w-0 space-y-6 w-full">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 mb-2">
                  <h1 className="text-xl sm:text-2xl font-semibold tracking-tight truncate">{company.name}</h1>
                  <ScoreBadge score={company.score} size="md" showLabel />
                </div>
                <a href={company.website} target="_blank" rel="noreferrer" className="text-sm text-primary hover:underline flex items-center gap-1 truncate">
                  {company.website} <ExternalLink className="h-3 w-3 flex-shrink-0" />
                </a>
              </div>
              <div className="flex gap-2 flex-shrink-0 w-full sm:w-auto">
                <Button variant="outline" size="sm" className="gap-1.5 flex-1 sm:flex-none">
                  <Bookmark className="h-3.5 w-3.5" /> <span className="hidden sm:inline">Save</span>
                </Button>
                <Button
                  size="sm"
                ...
                >
                  ...
                  <span className="hidden sm:inline">{enriching ? 'Enriching...' : company.enriched ? 'Re-Enrich' : 'Enrich'}</span>
                  <span className="sm:hidden text-xs">{enriching ? '...' : 'Enrich'}</span>
                </Button>
              </div>
            </div>
          </div>
          ...
          <div className="card-elevated p-4 sm:p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 text-sm">
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="font-medium text-foreground">{company.stage}</span>
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="font-medium text-foreground">Founded {company.foundedYear}</span>
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
                <Users className="h-3.5 w-3.5 flex-shrink-0" /> 
                <span className="font-medium text-foreground">{company.employees} employees</span>
              </div>
            </div>
          ...
          {company.enriched && company.enrichmentData && (
            <div className="space-y-6 slide-up">
              <div className="card-elevated p-4 sm:p-6">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">{company.enrichmentData.summary}</p>
              </div>
              ...
              <div className="card-elevated p-4 sm:p-6">
                ...
                <div className="space-y-2">
                  {company.enrichmentData.sources.map((src, i) => (
                    <div key={i} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-sm">
                      <a href={src.url} target="_blank" rel="noreferrer" className="text-primary hover:underline flex items-center gap-1 truncate">
                        <Link2 className="h-3 w-3 flex-shrink-0" /> <span className="truncate">{src.url}</span>
                      </a>
          ...
          <div className="card-elevated p-4 sm:p-6">
            ...
            <div className="space-y-3 mb-4 max-h-96 overflow-y-auto">
              ...
              {notes.map((note) => (
                <div key={note.id} className="flex items-start justify-between gap-2 rounded-lg bg-secondary p-3">
                  ...
                  <button onClick={() => deleteNote(note.id)} className="text-muted-foreground hover:text-destructive shrink-0 mt-1">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              />
              <Button size="sm" onClick={handleAddNote} disabled={!noteText.trim()} className="w-full sm:w-auto sm:self-end flex-shrink-0">
                Add
              </Button>
            </div>
          </div>
